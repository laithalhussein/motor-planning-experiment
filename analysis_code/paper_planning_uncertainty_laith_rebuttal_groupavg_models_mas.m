...
modb.alpha = regress(modb.Y - modb.no_ob, [modb.ob - modb.no_ob]); modb.alpha1 = min(max(modb.alpha,0),1);
moda.alpha = regress(moda.Y - modb.no_ob, [moda.ob - moda.no_ob]); moda.alpha1 = min(max(moda.alpha,0),1);
modb.beta = regress(modb.Y-modb.no_ob, (-modb.ob-15)); modb.beta1 = min(max(modb.beta,0),1);
