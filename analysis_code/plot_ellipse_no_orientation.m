function [ h, orient_deg ] = plot_ellipse_no_orientation(x,y,col,LW, error_type)
...
elseif regexpi(error_type, 'standard_error')
    ...
    e_points = [cos(a),sin(a)] * [num_errors / sqrt(num_samples)];
    %H=sqrt(eval)*evec' * num_errors * [num_errors / sqrt(num_samples)];
end

h = plot((e_points(:,1)*nanstd(x))+nanmean(x), (e_points(:,2)*nanstd(y))+nanmean(y),'color',col,'linewidth',LW); %
